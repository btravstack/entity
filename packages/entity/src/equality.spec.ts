...
test("distinct instances with the same data are equal", () => {
  const a = Organization.make(raw).getOrThrow();
  const b = Organization.make(raw).getOrThrow();
...
test("differing data is unequal", () => {
  const a = Organization.make(raw).getOrThrow();
  const b = Organization.make({ ...raw, slug: "other" }).getOrThrow();
  expect(a.equals(b)).toBe(false);
});

test("entities holding equal arrays are equal", () => {
  // This is the case a shallow, reference-comparing rule got wrong.
  const a = Organization.make({ ...raw, tags: ["x", "y"] }).getOrThrow();
  const b = Organization.make({ ...raw, tags: ["x", "y"] }).getOrThrow();
  expect(a.toJSON().tags === b.toJSON().tags).toBe(false);
  expect(a.equals(b)).toBe(true);
});

test("different entity types with identical data are unequal", () => {
  expect(Organization.make(raw).getOrThrow().equals(Team.make(raw).getOrThrow())).toBe(false);
});
